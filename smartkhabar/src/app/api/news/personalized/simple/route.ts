...
export const GET = withErrorHandling(async (request: NextRequest, context) => {
    ...
    try {
      ...
      if (prefsData.success && prefsData.preferences) {
        userPreferences = {
          ...
          readingTime: prefsData.preferences.readingTime === 'short' ? 3 : 
                      prefsData.preferences.readingTime === 'long' ? 10 : 5,
          preferredSources: prefsData.preferences.sources || [],
          excludedSources: [],
        ...
        throw new Error('Failed to get fallback preferences');
      }
    } catch (fallbackError) {
  ...
  // Get articles from database using simple SQL queries
